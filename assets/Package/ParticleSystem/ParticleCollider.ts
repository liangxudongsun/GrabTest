...
import { CCEManager, PanelManager, GizmoPanel, ReactiveStore ,ColliderData} from '../../Plans/index';
const { ccclass, property, executeInEditMode,menu } = _decorator;

const { Ray, AABB, OBB, intersect } = geometry;

function panelId(self: any) { return PanelManager.id(self.node, 'ParticleColliderPanel'); }

@ccclass('ParticleCollider')
@menu('Package/ParticleCollider')
...
export class ParticleCollider extends Component {
    ...
    private _store = new ReactiveStore<ColliderData>({
        mode: 'box', name: 'size',
        size: {
            x: { val: this.size.x, max: 50, min: 0.1, name: 'X' },
            y: { val: this.size.y, max: 50, min: 0.1, name: 'Y' },
            z: { val: this.size.z, max: 50, min: 0.1, name: 'Z' },
        },
        radius: { x: { val: this.R, max: 50, min: 0.1, name: 'R' } },
        offset: {
            x: { val: this.offset.x, max: 50, min: -50, name: 'X' },
            y: { val: this.offset.y, max: 50, min: -50, name: 'Y' },
            z: { val: this.offset.z, max: 50, min: -50, name: 'Z' },
        },
        rot: {
            x: { val: this.rotation.x, max: 360, min: 0, name: 'X' },
            y: { val: this.rotation.y, max: 360, min: 0, name: 'Y' },
            z: { val: this.rotation.z, max: 360, min: 0, name: 'Z' },
        },
        groupTag: { name: 'test2', data: String(this.group) },
    });
    ...
    start() {
        ...
        // 用序列化值初始化 store
        this._store.set({ mode: 'box', name: 'size', size: { x: { val: this.size.x, max: 50, min: 0.1, name: 'X' }, y: { val: this.size.y, max: 50, min: 0.1, name: 'Y' }, z: { val: this.size.z, max: 50, min: 0.1, name: 'Z' } }, radius: { x: { val: this.R, max: 50, min: 0.1, name: 'R' } }, offset: { x: { val: this.offset.x, max: 50, min: -50, name: 'X' }, y: { val: this.offset.y, max: 50, min: -50, name: 'Y' }, z: { val: this.offset.z, max: 50, min: -50, name: 'Z' } }, rot: { x: { val: this.rotation.x, max: 360, min: 0, name: 'X' }, y: { val: this.rotation.y, max: 360, min: 0, name: 'Y' }, z: { val: this.rotation.z, max: 360, min: 0, name: 'Z' } }, groupTag: { name: 'test2', data: String(this.group) } });

        // 响应式数据变化 → 更新组件 
        this._unsubStore = this._store.subscribe(d => {
            if (d.size.x) this.size.set(d.size.x.val, d.size.y.val, d.size.z.val);
            if (d.radius?.x) this.R = d.radius.x.val;
    ...
    lateUpdate() {
        ...
        if (!this._unsubStore) {
            ...
            // 立即用当前 store 值同步组件属性，避免被后续同步代码覆盖
            const cur = this._store.get();
            if (cur.size) this.size.set(cur.size.x.val, cur.size.y.val, cur.size.z.val);
            if (cur.radius?.x) this.R = cur.radius.x.val;
            if (cur.offset) this.offset.set(cur.offset.x.val, cur.offset.y.val, cur.offset.z.val);
            if (cur.rot) this.rotation.set(cur.rot.x.val, cur.rot.y.val, cur.rot.z.val);
        }
        ...
        if (this._dragging) {
            ...
            this._store.set({ size: { x: { val: ns.x, max: 50, min: 0.1, name: 'X' }, y: { val: ns.y, max: 50, min: 0.1, name: 'Y' }, z: { val: ns.z, max: 50, min: 0.1, name: 'Z' } } });
        }

        // 同步 Inspector 手动修改 → store
        const d = this._store.get();
        if (d.size && (this.size.x !== d.size.x.val || this.size.y !== d.size.y.val || this.size.z !== d.size.z.val)) {
            this._store.set({ size: { x: { val: this.size.x, max: 50, min: 0.1, name: 'X' }, y: { val: this.size.y, max: 50, min: 0.1, name: 'Y' }, z: { val: this.size.z, max: 50, min: 0.1, name: 'Z' } } });
        }
        if (d.radius?.x && this.R !== d.radius.x.val) {
            this._store.set({ radius: { x: { val: this.R, max: 50, min: 0.1, name: 'R' } } });
        }
        if (d.offset && (this.offset.x !== d.offset.x.val || this.offset.y !== d.offset.y.val || this.offset.z !== d.offset.z.val)) {
            this._store.set({ offset: { x: { val: this.offset.x, max: 50, min: -50, name: 'X' }, y: { val: this.offset.y, max: 50, min: -50, name: 'Y' }, z: { val: this.offset.z, max: 50, min: -50, name: 'Z' } } });
        }
        if (d.rot && (this.rotation.x !== d.rot.x.val || this.rotation.y !== d.rot.y.val || this.rotation.z !== d.rot.z.val)) {
            this._store.set({ rot: { x: { val: this.rotation.x, max: 360, min: 0, name: 'X' }, y: { val: this.rotation.y, max: 360, min: 0, name: 'Y' }, z: { val: this.rotation.z, max: 360, min: 0, name: 'Z' } } });
        }
        ...
        if (mode === 'sphere') {
            const radius = this._store.get().radius?.x?.val ?? 1;
            gr.addSphere(pos, radius, new Color(0, 255, 0, 64), 24, undefined, false);
