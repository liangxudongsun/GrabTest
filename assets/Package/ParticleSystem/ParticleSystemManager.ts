    ...
    private _resolveParticleParticle() {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            for (let ci = 0; ci < candidates.length; ci++) {
                ...
                const pull = overlap / (mA + mB) * 0.15;
                a.particle.position.x -= nx * pull * mB;
                a.particle.position.y -= ny * pull * mB;
                a.particle.position.z -= nz * pull * mB;
                b.particle.position.x += nx * pull * mA;
                b.particle.position.y += ny * pull * mA;
                b.particle.position.z += nz * pull * mA;
                ...
                if (relVn >= 0) {
                    // 正在分离但依旧重叠 → 轻推速度防止粘滞
                    const sep = Math.min(overlap * 0.05, 0.2);
                    a.particle.velocity.x -= nx * sep * (mB / (mA + mB));
                    a.particle.velocity.y -= ny * sep * (mB / (mA + mB));
                    a.particle.velocity.z -= nz * sep * (mB / (mA + mB));
                    b.particle.velocity.x += nx * sep * (mA / (mA + mB));
                    b.particle.velocity.y += ny * sep * (mA / (mA + mB));
                    b.particle.velocity.z += nz * sep * (mA / (mA + mB));
                    continue;
                }
