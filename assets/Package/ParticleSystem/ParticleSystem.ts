import { _decorator, Component, Vec3, Quat, Color, CurveRange, GradientRange, Material, Mesh, MeshRenderer, gfx, director, geometry, utils, editorExtrasTag, Vec4, Enum } from 'cc';
...
import { refreshOnSceneChange } from '../Tool';

const { ccclass, property ,executeInEditMode,menu } = _decorator;
@ccclass('ParticleSystem')
@menu('Package/ParticleSystem')
@executeInEditMode(true)
@refreshOnSceneChange
export class ParticleSystem extends Component implements IParticleSystem{
    // ─── 容量 ───
    @property({ tooltip: '最大粒子数' })
    capacity = 100;

    // ─── 发射 ───
    @property({ tooltip: '每秒发射的粒子数，0.25 = 每4秒1个' })
    rateOverTime = 10;
    @property({ tooltip: '粒子系统的总运行时长，-1 = 无限循环' })
    duration = -1;
    @property({ tooltip: '结束后重新开始' })
    loop = true;
    @property({ tooltip: '加载后自动播放' })
    playOnAwake = true;

    // ─── 初始值 ───
    @property({ tooltip: '初速度' })
    startSpeed = 1;
    @property({ tooltip: '初始大小' })
    startSize = 1;
    @property({ tooltip: '粒子存活时间（秒）' })
    startLifetime = 1;
    @property({ tooltip: '初始颜色' })
    startColor = new Color(255, 255, 255, 255);
    @property({ tooltip: '粒子质量，影响碰撞' })
    startMass = 1;

    // ─── 形状 ───
    @property({ type: Enum(EmitShape), tooltip: '粒子发射形状' })
    shape: EmitShape = EmitShape.POINT;
    @property({ tooltip: '形状尺寸 / 半径' })
    shapeRadius = 1;
    ...
    play() {
        ...
        this._stopped = false;
        this._elapsed = 0;
        this._emitAccum = 1;    // 启动时立即发射第一个粒子
    }
